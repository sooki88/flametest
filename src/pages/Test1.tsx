...
import { ShareholderListType } from "../types/test1.type";
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-20 p-5">
      ...
      <ul className="flex flex-col gap-10">
        {shareholderList?.map((list, index) => (
          <li key={index} className="relative flex w-full items-center gap-20 rounded bg-gray-200 px-8 py-10">
            <span className="absolute left-8 top-[-10px]">주주</span>
            <div className="flex flex-col gap-5">
              <label htmlFor={`sharholder${index}name`}>이름</label>
              <input
                id={`sharholder${index}name`}
                type="text"
                value={list.name}
                onChange={(e) => onChangeList(e, index, "name")}
                placeholder="주주명"
                required
                className="rounded border border-black px-2"
              />
            ...
            <div className="flex flex-col gap-5">
              ...
              <input
                id={`sharholder${index}shareNum`}
                type="number"
                value={list.shareNum ?? ""}
                onChange={(e) => onChangeList(e, index, "shareNum")}
                placeholder="0"
                required
                className="rounded border border-black px-2"
              />
      ...
      <div className="flex w-full items-center justify-between">
        <p>
          합산 지분율 : {ratioSum ?? "지분율의 합"} <span className="text-base text-red-500">{errMsgRatioSum}</span>
        </p>
